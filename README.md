...
- **[`docs/EDGE_VERDICT.md`](docs/EDGE_VERDICT.md)** — the honest no-edge verdict: 0/4 OOS folds on the 24h fixture (PBO = 1.00) and 0/4 OOS folds on a 2026-09-10 six-month run (PBO = 0.00 — read that page before treating the PBO drop as good news). That rejection *is* the platform working: it tells you a signal is dead so you move on, instead of shipping an overfit backtest.

## Results (honest one-liner)

**24h fixture (1,440 bars): a genuine gross per-trade edge that trading costs eat alive — the net-of-cost gate flips it net-positive on this short window. Six-month run (264,961 bars, 2026-09-10): no gross edge and no out-of-sample edge — in-sample Sharpe is already strongly negative before walk-forward gets involved.** Both are *simulated* results, not live-trading claims. Both are published; the six-month run carries the statistical weight and supersedes the 24h numbers as the load-bearing evidence without erasing them — see [docs/EDGE_VERDICT.md](docs/EDGE_VERDICT.md).

- **Edge verdict (read this first):** [`docs/EDGE_VERDICT.md`](docs/EDGE_VERDICT.md) — run to a conclusion with the project's own tooling. **24h fixture:** 0/4 walk-forward folds profitable, PBO = 1.00 (a degenerate tie-break on near-tied OOS Sharpes, not an independent measurement — see the page). **Six-month run (2026-09-10, 264,961 bars):** still 0/4 folds profitable, total OOS PnL −18,845.07, OOS Sharpe −19.29 ± 10.54, Deflated Sharpe 0.0005 with P(true Sharpe ≤ 0) = 0.9995, and PBO drops to 0.00 — **not because the strategy improved**, but because every config loses roughly equally, so there is no in-sample winner for PBO to catch falling into the OOS bottom half. Reporting a negative result honestly, on both windows, is the point.
- **Full results & caveats:** [`docs/RESULTS.md`](docs/RESULTS.md) — unedited backtester/calibrator/walk-forward output for the 24h fixture, the six-month walk-forward run, the buy-and-hold benchmark, and an explicit honest-caveats block.
- **Performance tearsheet:** [`docs/assets/tearsheet.svg`](docs/assets/tearsheet.svg) — equity curve, underwater drawdown, calibrate threshold sweep, and walk-forward OOS PnL for the 24h fixture, every figure verbatim from `docs/RESULTS.md`.
- **Cost-sweep frontier:** the [parameter-sweep table in `docs/RESULTS.md`](docs/RESULTS.md#parameter-sweep-calibrator) (rendered as the threshold-sweep panel of the tearsheet, 24h fixture) shows PnL getting *less* negative as the strategy trades less — the fingerprint of cost dominance. The six-month run's fold 4 shows the same fingerprint at larger scale: the highest-turnover fold (18,224 fills) also carries the largest single-fold OOS loss.
...
- **What this tests:** `obi` in the backtest is signed taker flow computed from executed trades, not the L2 order-book depth imbalance the live stack trades. See [docs/ROADMAP.md](docs/ROADMAP.md) for the parity gap and the plan to close it.
...
<!-- Generated by scripts/gen-tearsheet.py from docs/RESULTS.md. Regenerate with: python3 scripts/gen-tearsheet.py -->
<p align="center">
  <img src="docs/assets/tearsheet.svg" alt="Performance tearsheet for the OBIThreshold(0.70) strategy: equity curve, underwater drawdown, calibrate threshold sweep, and walk-forward out-of-sample PnL, with a stats table. Every figure is taken verbatim from docs/RESULTS.md and labelled as a simulated backtest on a ~24h window, not a live-trading result." width="100%">
</p>

Every number on the tearsheet is taken **verbatim from [docs/RESULTS.md](docs/RESULTS.md)** — the unedited backtester/calibrator/walk-forward output — and is a *simulated* result on a short ~24h window, not a live-trading claim. The honest headline: OBI lands ≈ breakeven net, fee-dominated (PnL gets *less* negative as it trades less), and under proper walk-forward **0 of 4 out-of-sample folds are profitable**. That negative walk-forward result is the one with methodological weight.

### Provable metrics (computed at build time)

<!-- Generated by scripts/gen-metrics-panel.py from the working tree. Regenerate with: python3 scripts/gen-metrics-panel.py -->
<p align="center">
  <img src="docs/assets/metrics.svg" alt="Provable repository metrics panel: lines of code, automated test count, docker-compose service count, signal-layer count, service-repo count, and ADR count — each computed at build time from the working tree and labelled with the method used to derive it." width="100%">
</p>

These figures are **computed at build time** by [`scripts/gen-metrics-panel.py`](scripts/gen-metrics-panel.py) (LOC via a cloc-like pass, test count via grep of `func Test` / `def test_` / `@Test`, service count parsed from `docker-compose.yml`, signal layers parsed from the README) — generated, not claimed. Any figure the script cannot compute is omitted rather than invented.
...
- Reuses `huginn/internal/research` — the same engine `cmd/walkforward` uses — so a gateway run and a CLI run share one code path, and it reproduces the honest result (24h fixture: OBI 0/4 OOS folds, PBO = 1.00, total OOS PnL −146.11; six-month run, 2026-09-10: 0/4 OOS folds, PBO = 0.00, total OOS PnL −18,845.07 — see [docs/EDGE_VERDICT.md](docs/EDGE_VERDICT.md))
...
- **What it demonstrates:** the HMM/Baum-Welch regime-modelling lineage that Renaissance's Medallion fund is documented to have used — implemented honestly, as a descriptive lens with no edge claim (0/4 OOS-profitable folds on both the 24h fixture and the six-month run — see [docs/EDGE_VERDICT.md](docs/EDGE_VERDICT.md))
...
- **[Production Readiness Scorecard](docs/PRODUCTION_READINESS.md)** — honest DONE/GAP audit across reliability, security, observability, data integrity, testing, and quant rigor, with what would change for a real-money deployment
