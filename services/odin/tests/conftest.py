"""Test bootstrap for Odin.

odin.py imports `kafka` at module import time. The analytics logic under test
(PerformanceTracker) does not touch Kafka, so we install lightweight stub
modules for `kafka` / `kafka.errors` before importing odin. This lets the unit
...
# Make services/odin importable as `odin`.
_ODIN_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _ODIN_DIR not in sys.path:
    sys.path.insert(0, _ODIN_DIR)
